function carouselInit() {
  document.querySelectorAll("[data-carousel]").forEach((carousel) => {
    ...
    nextBtn?.addEventListener("click", () => {
      if (isStatic()) return;
      const itemsPerView = getItemsPerView();
      const nextIndex = (Math.floor(index / itemsPerView) + 1) * itemsPerView;
    ...
    prevBtn?.addEventListener("click", () => {
      if (isStatic()) return;
      const itemsPerView = getItemsPerView();
      const prevIndex = (Math.floor(index / itemsPerView) - 1) * itemsPerView;
  ...
  tabsBlocks.forEach((tabsBlock) => {
    ...
    tabsLinks.forEach((link) => {
      link.addEventListener("click", (event) => {
        ...
        const activeContent = tabsBlock.querySelector(
          `.js-tabs-content[data-tab="${tabIndex}"]`,
        );
        if (activeContent) {
          activeContent.classList.add("is-active");
        }
        const activePic = tabsBlock.querySelector(
          `.js-tabs-pic[data-tab="${tabIndex}"]`,
        );
    ...
    if ($phoneInputs.length > 0) {
      $phoneInputs.forEach((phoneInput) => {
        ...
        if ($dialcode) {
          ...
          const $dialcodeFlag =
            $dialcodeShow.querySelector(".c-dialcode__flag");
          const $dialcodeOptions =
            $dialcode.querySelectorAll(".js-dialcodeOption");
          let currentPrefix = $prefixEl.textContent;
          if ($dialcodeShow) {
            $dialcodeShow.addEventListener("click", (e) => {
              e.stopPropagation();
              document
                .querySelectorAll(".js-dialcode.is-open")
                .forEach((openDialcode) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const contactMethodHiddenInput = form.querySelector(
    'input[name="primary-contact-method-final"]',
  );
  ...
  if (!successMessage) {
    ...
    successMessage.textContent =
      "¡Gracias! Tu solicitud ha sido enviada exitosamente. Te contactaremos pronto.";
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f4f4f4;">
  ...
  async function submitForm(data) {
    const url =
      "https://l5j6mnkgyvi3pvrcqzhenb5fc40rwipi.lambda-url.us-east-1.on.aws/";
  ...
  async function handleSubmit(event) {
    ...
    const phone = phoneHiddenInput
      ? phoneHiddenInput.value
      : phoneInput
        ? phoneInput.value
        : "";
    const contactMethod = contactMethodHiddenInput
      ? contactMethodHiddenInput.value
      : contactMethodInput
